import Link from 'next/link';
import { fetchAPI } from '@/lib/strapi';
import { ROUTES, SITE_NAME, WHATSAPP_NUMBER } from '@/lib/constants';
import type { GlobalSetting } from '@/types/cms';

async function getGlobalSetting(): Promise<GlobalSetting | null> {
  try {
    const data = await fetchAPI<{ data: GlobalSetting }>({
      endpoint: '/global-setting',
      revalidate: 300,
      tags: ['global-setting'],
    });
    return data.data || null;
  } catch {
    return null;
  }
}

export async function Footer() {
  const settings = await getGlobalSetting();
  const currentYear = new Date().getFullYear();

  const storeName = settings?.storeName || SITE_NAME;
  const phone = settings?.phone || null;
  const email = settings?.email || null;
  const address = settings?.address || null;
  const city = settings?.city || 'Montería';
  const department = settings?.department || 'Córdoba';
  const whatsapp = settings?.whatsappNumber || WHATSAPP_NUMBER;
  const whatsappMsg = settings?.whatsappMessage || 'Hola, me interesa hacer un pedido';
  const facebook = settings?.facebookUrl || null;
  const instagram = settings?.instagramUrl || null;
  const tiktok = settings?.tiktokUrl || null;

  return (
    <footer className="border-border bg-primary border-t text-white">
      <div className="mx-auto max-w-7xl px-4 py-12">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {/* Brand */}
          <div>
            <div className="mb-4 flex items-center gap-2">
              <div className="bg-accent flex h-10 w-10 items-center justify-center rounded-lg text-lg font-bold">
                M
              </div>
              <div>
                <h3 className="text-lg font-bold">{storeName}</h3>
                <p className="text-xs text-gray-400">Venta al por Mayor</p>
              </div>
            </div>
            <p className="mb-4 text-sm leading-relaxed text-gray-400">
              Tu tienda mayorista de confianza en {city}, {department}. Los mejores precios en ropa,
              calzado y accesorios con envíos a todo Colombia.
            </p>
            {(facebook || instagram || tiktok) && (
              <div className="flex items-center gap-3">
                {facebook && (
                  <a
                    href={facebook}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Facebook"
                    className="text-gray-400 transition-colors hover:text-white"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                  </a>
                )}
                {instagram && (
                  <a
                    href={instagram}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Instagram"
                    className="text-gray-400 transition-colors hover:text-white"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                    </svg>
                  </a>
                )}
                {tiktok && (
                  <a
                    href={tiktok}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="TikTok"
                    className="text-gray-400 transition-colors hover:text-white"
                  >
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.28 6.28 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.18 8.18 0 004.78 1.52V6.77a4.86 4.86 0 01-1.01-.08z" />
                    </svg>
                  </a>
                )}
              </div>
            )}
          </div>

          {/* Tienda */}
          <div>
            <h4 className="mb-4 text-sm font-semibold tracking-wider text-gray-400 uppercase">
              Tienda
            </h4>
            <ul className="space-y-2 text-sm">
              {[
                { href: ROUTES.PRODUCTS, label: 'Catálogo' },
                { href: `${ROUTES.CATEGORIES}/ropa`, label: 'Ropa' },
                { href: `${ROUTES.CATEGORIES}/calzado`, label: 'Calzado' },
                { href: `${ROUTES.CATEGORIES}/accesorios`, label: 'Accesorios' },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="hover:text-accent-light text-gray-300 transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Ayuda */}
          <div>
            <h4 className="mb-4 text-sm font-semibold tracking-wider text-gray-400 uppercase">
              Ayuda
            </h4>
            <ul className="space-y-2 text-sm">
              {[
                { href: ROUTES.ABOUT, label: 'Quiénes Somos' },
                { href: ROUTES.CONTACT, label: 'Contacto' },
                { href: ROUTES.POLICIES, label: 'Políticas' },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="hover:text-accent-light text-gray-300 transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contacto */}
          <div>
            <h4 className="mb-4 text-sm font-semibold tracking-wider text-gray-400 uppercase">
              Contacto
            </h4>
            <ul className="space-y-3 text-sm">
              <li className="flex items-start gap-2 text-gray-300">
                <span>📍</span>
                <span>{address || `${city}, ${department}, Colombia`}</span>
              </li>
              {phone && (
                <li>
                  <a
                    href={`tel:${phone}`}
                    className="hover:text-accent-light flex items-center gap-2 text-gray-300 transition-colors"
                  >
                    <span>📞</span> {phone}
                  </a>
                </li>
              )}
              {email && (
                <li>
                  <a
                    href={`mailto:${email}`}
                    className="hover:text-accent-light flex items-center gap-2 text-gray-300 transition-colors"
                  >
                    <span>✉️</span> {email}
                  </a>
                </li>
              )}
              {whatsapp && (
                <li>
                  <a
                    href={`https://wa.me/${whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(whatsappMsg)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-gray-300 transition-colors hover:text-green-400"
                  >
                    <span>💬</span> WhatsApp
                  </a>
                </li>
              )}
            </ul>
          </div>
        </div>

        {/* Bottom */}
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-6 sm:flex-row">
          <p className="text-xs text-gray-500">
            © {currentYear} {storeName}. Todos los derechos reservados.
          </p>
          <div className="flex items-center gap-2">
            {['💳 Wompi', '🏦 PSE', '📱 Nequi'].map((badge) => (
              <span key={badge} className="rounded bg-white/10 px-2 py-1 text-[10px] text-gray-400">
                {badge}
              </span>
            ))}
          </div>
        </div>
      </div>
    </footer>
  );
}
